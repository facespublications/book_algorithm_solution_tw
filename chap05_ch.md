...
這是由部分和問題稍微延伸的問題。透過針對部分和問題的動態規劃法，可以作出一個陣列，用來表示是否能夠以O(NW)的計算複雜度分別合成1、2、...、W。藉此，求出在1、2、...、W中可合成的值有多少個。關於具體的演算法，記載於以下文章。

[AtCoder TDPC A - 競賽](https://drken1215.hatenablog.com/entry/2020/12/21/153600)
...
這個問題比5.5（不限制個數的部分和問題）更加困難。在5.5中，可以無限制的使用各整數，但這題制定了使用每個整數  `a[i]` 的次數上限 `m[i]` 。因此，將陣列 dp 陣列的定義修改如下。
...
本問題是第5.5節解說的「求出編輯距離的問題」的類似題。與求出編輯距離的動態規劃法同樣，定義如下陣列 `dp`。另外，最長共同子序列通常稱為 LCS（Longest Common Subsequence）。
...
本問題是第5.6節中解說的「區間分割最佳化問題」的一個類似題。關於這個問題的解決方案，在以下文章中進行詳細解說。
...
對滿足 i < k < j 的每個 k，進行以上的狀況區分。因此，根據動態規劃法的更新部分可以表示如下（注意實際上 `S[i:j]` 這種表示在 C++ 中是不存在的）。
...
最後，考慮關於動態規劃法的更新順序。在上述的轉移式中，以已知 `dp[i][k]`和 `dp[k][j]` 的值為前提，將 `dp[i][j]` 的值進行更新。在此，k - i 和 j - k 都比 j - i 小。因此，更新陣列 `dp[i][j]` 時，只需要按照 j - i 的大小順序進行更新即可。
