...
對於無向圖 G 的最小生成樹 T 和其所包含的邊 e，將關於 T , e 的基本割集設為 C。將除了 C 所包含的 e 以外的任意邊設為 f 。此時，T' = T - e + f 也形成生成樹。由於 T 是最小生成樹，形成如下式。
...
此外，作為解決問題的具體演算法，應用一般求最小生成樹的 Kruskal 演算法即可。時間複雜度為 O(N log M)。
